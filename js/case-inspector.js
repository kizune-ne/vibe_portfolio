...
• Проект оформлен в аккуратный открытый репозиторий с подробным Readme и файловым проводником прошивки.`,
  ...
  'ai-influencer': {
    ...
    subtitle: 'Полный пайплайн разработки персонажа: генерация референсов лица (Nanobanano 2), составление датасета и капшенинга (Wan 2.2 / Flux), локальная тренировка LoRA на RTX 4070 Ti SUPER в AI-Toolkit и финальный сэмплинг в ComfyUI.',
    role: 'Generative AI Specialist / Synthetic Media Developer',
    metrics: [
      { value: '100%', label: 'Character Consistency' },
      { value: '16 GB', label: 'VRAM RTX 4070 Ti SUPER' },
      { value: 'Wan 2.2', label: 'Архитектура датасета' },
      { value: 'AI-Toolkit', label: 'Локальный тренинг' }
    ],
    problem: `При стандартной генерации медиаконтента с помощью нейросетей по обычным текстовым промптам возникают критические проблемы:
• Невозможность сохранить уникальное лицо персонажа от кадра к кадру (Character Drift / размытие геометрии лица).
• Высокие затраты и лимиты при использовании сторонних облачных генераторов и отсутствие полного контроля над весами модели.`,
    solution: `Разработан 4-этапный автономный производственный пайплайн:
1. **Концептуализация и Референсы**: Создание каноничного портрета аватара через Nanobanano 2, ComfyUI воркфлоу и ИИ-чаты с ручным фильтром идеальных каноничных кадров.
2. **Формирование Датасета & Капшенинг**: Сбор сета высокого разрешения, кадрирование и формирование текстовых описаний (captioning) с адаптацией под Wan 2.2 и Flux.
3. **Локальная Тренировка LoRA (AI-Toolkit)**: Запуск обучения LoRA на локальном железе (AMD Ryzen 7 9700X + NVIDIA GeForce RTX 4070 Ti SUPER 16GB VRAM) через AI-Toolkit с ручной проверкой контрольных точек.
4. **Сэмплирование в ComfyUI**: Подключение обученной LoRA к адаптированным нодовым воркфлоу ComfyUI для массовой генерации сюжетных сетов с 100% сохранением аватара.`,
    results: `• Полная независимость от облачных сервисов благодаря локальному обучению моделей.
• Достижение 100% сопоставимости лица персонажа во всех ракурсах и сложных сюжетных локациях.
• Готовый масштабируемый процесс создания визуального контента под соцсети и бренды.`,
    techStack: [
