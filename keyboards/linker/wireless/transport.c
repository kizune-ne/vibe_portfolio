...
void set_transport(transport_t new_transport) {
    ...
    bool force = false;
    if (new_transport == TRANSPORT_WLS && host_get_driver() == &chibios_driver) force = true;
    if (new_transport == TRANSPORT_USB && host_get_driver() == &wireless_driver) force = true;

    if (transport != new_transport || force) {
