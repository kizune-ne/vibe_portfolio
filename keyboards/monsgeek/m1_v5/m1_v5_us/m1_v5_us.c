...
static uint32_t startup_white_timer = 0;  // Таймер для отсчета 10 секунд
static bool trigger_startup_white = false; // Флаг-переключатель белого света
// -----------------------------
...
#    endif
	...
	// --- НАШ КОД СТАРТА ---
    trigger_startup_white = true;         // Даем добро на белый свет
    startup_white_timer = timer_read32(); // Засекаем текущее время
    // ----------------------
	}
...
void bat_indicators(void) {
    ...
    static bool prev_charging_state = false; // Запоминаем прошлое состояние кабеля
    uint8_t bat_level = *md_getp_bat();      // Получаем текущий точный процент заряда

    // --- ЛОГИКА ПОДКЛЮЧЕНИЯ КАБЕЛЯ ---
    // Проверяем: если кабель сейчас подключен, а секунду назад был отключен
    if (charging_state && !prev_charging_state) {
        if (bat_level >= 90 || bat_full_flag) { // И при этом батарея почти полная
            trigger_startup_white = true;       // Запускаем белый сигнал
            startup_white_timer = timer_read32(); // Обновляем таймер
        }
    }
    prev_charging_state = charging_state; // Сохраняем состояние для следующей проверки

    // --- 1. БЕЛЫЙ ПРИВЕТСТВЕННЫЙ СИГНАЛ (10 секунд) ---
    if (trigger_startup_white) {
        if (timer_elapsed32(startup_white_timer) < 10000) { // Пока не прошло 10000 мс (10 сек)
            if (bat_level >= 90 || bat_full_flag) { // Двойная проверка на высокий заряд
                rgb_matrix_set_color(HS_MATRIX_BLINK_INDEX_BAT, 0xFF, 0xFF, 0xFF); // Горим белым (255, 255, 255)
                return; // Выходим из функции, чтобы нижние правила не перекрасили кнопку
            }
        } else {
            // Время вышло, выключаем стартовый режим
            trigger_startup_white = false;
            startup_white_timer = 0;
        }
    }

    // --- 2. ПРОЦЕСС ЗАРЯДКИ ---
    if (charging_state) {
        if (bat_level >= 90 || bat_full_flag) {
            // Заряжено (>90%) и 10 секунд уже прошли -> ничего не делаем.
            // Индикатор не получает цвета и просто возвращается к штатной подсветке
            battery_process_time = 0;
        } else {
            // Зарядка идет (заряд <90%) -> Горим жестким красным
            rgb_matrix_set_color(HS_MATRIX_BLINK_INDEX_BAT, 0xFF, 0x00, 0x00);
            battery_process_time = 0;
        }
    } 
    // --- 3. РАЗРЯЖЕНА (< 20%) ---
    else if (bat_level <= BATTERY_CAPACITY_LOW) { // BATTERY_CAPACITY_LOW мы задали как 20 в config.h
        // Функция мигания. Цифра 500 — это интервал мигания в миллисекундах (полсекунды). 
        // Если хочешь быстрее — поставь 250. Если медленнее — 1000.
        rgb_matrix_hs_bat_set(HS_MATRIX_BLINK_INDEX_BAT, (RGB){0xFF, 0x00, 0x00}, 1000, 1);

        // Встроенная защита от смерти аккумулятора
        if (bat_level <= BATTERY_CAPACITY_STOP) {
            if (!battery_process_time) {
                battery_process_time = timer_read32();
            }
            // Если заряд 0% держится 60 секунд -> рубим питание
    ...
    // --- 4. ШТАТНЫЙ РЕЖИМ (От 20% до 100% без кабеля) ---
    else {
        // Ничего не делаем, индикатор работает как обычная кнопка
        battery_process_time = 0;
    }
}
